...
columns_to_drop = ['etnia','cbo','municipio_residencia','evolucao_caso','etnia','raca_cor','ds','profissional_saude']
...
class Pre_Processing_Bases:
    def __init__(self, df, columns_geral, columns_symptoms, columns_to_drop):
        ...
        self.MAIN_SYMPTOMS = {'SYMPTOM_COUGH': ["TOSSE", "HEMOPTISE", "TOSSE PRODUTIVA", "GRIPADA"],
                            'SYMPTOM_COLD': ["ESPIRROS", "ESPIRRO", "ESPIRRANDO", "ESPIROS", "ESPIRO", "GRIPE", "GRIPAL", "SINTOMAS GRIPAIS", "SINDROME GRIPAL", "SINTOMAS DE GRIPE"],
                            'SYMPTOM_AIR_INSUFFICIENCY': ["DISPNEIA", "FALTA DE AR", "TAQUIPNEIA", "TAQUIDISPNEIA", "INSUFICIENCIA RESPIRATORIA"],
                            'SYMPTOM_FEVER': ["FEBRE"],
                            'SYMPTOM_LOW_OXYGEN_SATURATION': ["SATURACAO O2 < 95", "DESSATURACAO"],
        ...
        # self.categorizing_simptoms()
        # self.creating_data_symptoms()
        # self.run()

    def run(self,columns_geral,columns_symptoms,columns_to_drop):
    ...
    def categorizing_simptoms(self):
